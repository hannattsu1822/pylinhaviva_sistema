...
@main_bp.route('/frota/registrar-troca-oleo')
@login_required
def registrar_troca_oleo():
    with mysql.connection.cursor() as cur:
        cur.execute("SELECT id, placa, modelo FROM veiculos ORDER BY placa")
        veiculos = cur.fetchall()
    return render_template('frota/registrar_troca_oleo.html', veiculos=veiculos)


@main_bp.route('/frota/salvar-troca-oleo', methods=['POST'])
@login_required
def salvar_troca_oleo():
    try:
        data = request.form
        with mysql.connection.cursor() as cur:
            cur.execute("""
                INSERT INTO trocas_oleo_sky 
                (veiculo_id, data_troca, horimetro, responsavel_matricula, observacoes)
                VALUES (%s, %s, %s, %s, %s)
            """, (
                data['veiculo_id'],
                data['data_troca'],
    ...
    return redirect(url_for('main.registrar_troca_oleo'))
...
def checar_horimetro_frota():
    with mysql.connection.cursor() as cur:
        # Obter veículos com informações de horímetro
        cur.execute("""
            SELECT 
                v.id,
                v.placa,
                v.modelo,
                MAX(t.horimetro) as horimetro_ultima_troca,
                (SELECT i.horimetro FROM inspecoes i 
                 WHERE i.placa = v.placa 
                 ORDER BY i.data_inspecao DESC LIMIT 1) as horimetro_atual
            FROM veiculos v
            LEFT JOIN trocas_oleo_sky t ON v.id = t.veiculo_id
        ...
        for row in cur.fetchall():
            horimetro_para_troca = None
            status = "Sem dados"
            status_class = "secondary"
            progresso = 0

            if row['horimetro_ultima_troca'] is not None:
                ...
                if row['horimetro_atual'] is not None:
                    ...
                    if horas_consumidas >= 300:
                        ...
                        status_class = "danger"
                    elif horas_consumidas >= 250:  # Faltam 50h ou menos
            ...
            dados.append({
                ...
                'placa': row['placa'],
                'modelo': row['modelo'],
                'horimetro_ultima_troca': row['horimetro_ultima_troca'],
                'horimetro_atual': row['horimetro_atual'],
                'horimetro_para_troca': horimetro_para_troca,
                'status': status,
                'status_class': status_class,
                'progresso': progresso
            })

    return render_template('frota/checador_horimetro.html', dados=dados)
